...
func TestService_createOrGetInstance(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "instance does not exist (should create instance SEV_SNP) with confidential compute Enabled and InstanceType n2d",
			scope: func() Scope {
				machineScope.GCPMachine = getFakeGCPMachine()
				hostMaintenancePolicyTerminate := infrav1.HostMaintenancePolicyTerminate
				machineScope.GCPMachine.Spec.OnHostMaintenance = &hostMaintenancePolicyTerminate
				confidentialComputeEnabled := infrav1.ConfidentialComputePolicyEnabled
				machineScope.GCPMachine.Spec.ConfidentialCompute = &confidentialComputeEnabled
				machineScope.GCPMachine.Spec.InstanceType = "n2d-standard-4"
				return machineScope
			},
			mockInstance: &cloud.MockInstances{
				ProjectRouter: &cloud.SingleProjectRouter{ID: "proj-id"},
			...
			want: &compute.Instance{
				...
				MachineType: "zones/us-central1-c/machineTypes/n2d-standard-4",
				Metadata: &compute.Metadata{
					Items: []*compute.MetadataItems{
						{
							Key:   "user-data",
		...
		{
			name: "instance does not exist (should create instance SEV) with confidential compute Enabled and InstanceType c2d",
			scope: func() Scope {
				machineScope.GCPMachine = getFakeGCPMachine()
				hostMaintenancePolicyTerminate := infrav1.HostMaintenancePolicyTerminate
				machineScope.GCPMachine.Spec.OnHostMaintenance = &hostMaintenancePolicyTerminate
				confidentialComputeEnabled := infrav1.ConfidentialComputePolicyEnabled
				machineScope.GCPMachine.Spec.ConfidentialCompute = &confidentialComputeEnabled
				machineScope.GCPMachine.Spec.InstanceType = "c2d-standard-4"
				return machineScope
			},
			mockInstance: &cloud.MockInstances{
				ProjectRouter: &cloud.SingleProjectRouter{ID: "proj-id"},
			...
			want: &compute.Instance{
				...
				MachineType: "zones/us-central1-c/machineTypes/c2d-standard-4",
				Metadata: &compute.Metadata{
					Items: []*compute.MetadataItems{
						{
							Key:   "user-data",
